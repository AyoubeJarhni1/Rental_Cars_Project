...
public interface VoitureRepository extends JpaRepository<Voiture, Long> {
    ...
    @Query("SELECT v FROM Voiture v WHERE v.id IN (SELECT r.voiture.id FROM Reservation r WHERE r.dateDb <= :startOfWeek AND r.dateFin >= :endOfWeek)")
    List<Voiture> findRentedVehiclesBetweenDates(LocalDate startOfWeek, LocalDate endOfWeek);
